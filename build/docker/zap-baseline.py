...
>>>>>>> Pass openapi to -addoninstall, instead of passive-scan rules
    ...
    try:
      ...
      ls_output = subprocess.check_output(['docker', 'pull', 'stephendonner/zaproxy:1.0'])
    except OSError:
    ...
    try:
      ...
      if len(context_file) > 0:
        # Have to mount the directory containing the context file
  ...
  try:
    ...
    if len(context_file) > 0:
      # handle the context file, can't use base_dir as it might not have been set up
    ...
    if res.startswith("ZAP Error"):
      # errno.EIO is 5, not sure why my attempts to import it failed;)
